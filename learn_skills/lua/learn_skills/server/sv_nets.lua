...
net.Receive("skills_cmd", function(len, ply)
    ...
    if !cmd_args.target:IsValid() then return end
    if cmd_args.net_id == "auto_rpk" && !ply.__skills_rpk then
        ply.__skills_rpk = true
        local skills_info = {
        ...
        local message = {["color_1"] = Color(255, 255, 255), ["string_1"] = "Vous vous êtes auto Role Play Kill, une nouvelle aventure commence pour vous."}
        ...
        hook.Run("learn_skills_auto_rpk", ply, skills_info.Nature)
    elseif cmd_args.net_id == "auto_rpk" && ply.__skills_rpk then
        naruto_notif(ply, "Vous avez déjà fait un auto rpk aujourd'hui !", 1, 4)
    else
        if cmd_args.cmd && Learn_Skills.Config.Admin_Group[ply:GetUserGroup()] then
